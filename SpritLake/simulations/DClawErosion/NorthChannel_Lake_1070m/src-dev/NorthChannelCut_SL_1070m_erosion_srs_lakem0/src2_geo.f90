!
! New mass entrainment added by CJC 6/3/20
!
!=========================================================
...
!=========================================================
      ...
      double precision :: psi, dhdt, dmdt, tau_b, tau_d, theta_s, rhof_s, sig_s  ! ** new **
      integer :: i,j,ii,jj,jjend,icount,curvature
      logical :: ent

      double precision, allocatable :: moll(:,:)

      double precision, parameter :: sm    = 0.250d0
      double precision, parameter :: sms   = 0.360d0
      double precision, parameter :: smr   = 0.110d0
      !double precision, parameter :: alpha = 0.010d0
      double precision, parameter :: m_s   = 0.40d0
      double precision, parameter :: Cz    = 25.0d0
         ...
         do j=1-mbc+1,my+mbc-1
            ...
            if (ent.and.vnorm.gt.vlow.and.(aux(i,j,i_theta)>0.0)) then
               ...
               if (dbdv<slopebound.and.b_eroded<aux(i,j,i_theta)) then
                  ...
                  call mvg_model(sm,psi,sms,smr,alpha,m_s,theta_s)
                  psi = psi*98.1d0 ! convert from cm of head to Pascal
                  rhof_s = theta_s*1000.d0 + (1.0d0-theta_s)*1.220d0
                  m2 = (1.0d0-m_s)
                  rho2 = m2*2700.d0 + (1.0d0-m2)*rhof_s
                  tau_b = sigbed + rho*gmod*vnorm**2.0d0/Cz
                  sig_s = rho2*gmod*b_remaining - psi
                  tau_d = sig_s*tan(phi*pi/180.0d0)
                  dh = (tau_b/rho - tau_d/rho2)*dti**2.0d0
