...
#------------------------------
    ...
    topopath = os.path.join(topo,'Gorge','TannerCrk1','Tan1_118k_b_ascii')
    infile = 'tan1_118k_b_topo_with_slip_surface.tt3'
    
    bfile = os.path.join(topopath,infile)
    a=gt.topoboundary(bfile)
    xll = a[0] 
    yll = 5.0515e6 - 1.e3#a[2] 
    xu =  5.83e5#a[1]
    yu =  a[3] 

    header = gt.topoheaderread(bfile)
    cell = 1. #header['cellsize']
    coarsen = 1
    cell = cell*coarsen
    
    rc = 1 #factor of coarsening from DEM resolution

    r1 = 4 #=> number of coarse cells only
    r2 = 4 #=>level 2 = 16 meters
    r3 = 1 #=>level 3 = 4 meters, level 4 = 1 meter
    ...
    clawdata.restart_file = 'fort.chk0250'
...
def setgeo(rundata):
    ...
    topo=os.environ['TOPO']
    geodata.topofiles = []
    topopath = os.path.join(topo,'Gorge','TannerCrk1','tan_mp_sbs4B_ascii')
    infile = os.path.join(topopath,'tan_mp_sbs4b_topo_with_slip_surface.tt3')
    
    geodata.topofiles.append([3, 1, 3, -1.0, 1.e10, infile])
    ...
    initfile = os.path.join(topopath,'tan_mp_sbs4b_src_topo.tt3')
    geodata.qinitfiles.append([3,8,3,3,initfile])

    #initfile = os.path.join(topopath,'chi0_over_slipsurf.tt2')
    #geodata.qinitfiles.append([2,6,1,3,initfile])

    #initfile = os.path.join(topopath,'chi0_over_lake.tt2')
    #geodata.qinitfiles.append([2,6,1,3,initfile])
    ...
    #geodata.regions.append([1,3,-10.0,100.0,lake[0],lake[1],lake[2],lake[3]])
